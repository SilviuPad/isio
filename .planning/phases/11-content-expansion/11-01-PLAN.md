...
must_haves:
  truths:
    - "Web Development service page renders 800+ words including methodology, process, and FAQ"
    - "Web Apps service page renders 800+ words including methodology, process, and FAQ"
    - "SEO service page renders 800+ words including methodology, process, and FAQ"
    - "All service prose and FAQ content appears in both RO and EN"
  artifacts:
    - path: "src/lib/service-data.ts"
      provides: "ServiceDetail with prose and faq fields for web-development, web-apps, seo"
      contains: "prose"
    - path: "src/components/services/ServicePageLayout.astro"
      provides: "Renders prose paragraphs and FAQ accordion"
      contains: "prose"
  key_links:
    - from: "src/components/services/ServicePageLayout.astro"
      to: "src/lib/service-data.ts"
      via: "getServiceDetail() returns prose and faq fields"
      pattern: "section\\.prose"
---

<objective>
Extend the ServiceDetail data model with prose paragraphs and FAQ entries, update ServicePageLayout.astro to render them, and populate full bilingual content for the first three services (web-development, web-apps, seo).

Purpose: Service pages currently have ~150-200 words of feature-list content. SEO audit #12 flags them as critically thin. Adding methodology prose and FAQ sections brings each page to 800+ words of substantive content.

Output: Updated service-data.ts with prose/faq fields for 3 services, updated layout that renders prose + FAQ.
</objective>
...
From src/components/services/ServicePageLayout.astro:
- Receives `serviceKey` and `locale` as props
- Calls `getServiceDetail(serviceKey)` to get detail data
- Renders each `section` as an h2 + ServiceFeatureList (ul/li checkmarks)
- Has turnaround block and ServiceCTA at the end

From src/components/services/ServiceFeatureList.astro:
- Renders `items` as `<ul><li>` with checkmark SVG icons
- Takes `items: Array<{ text: { ro: string; en: string } }>` and `locale`
</interfaces>

<tasks>

<task type="auto">
  <name>Task 1: Extend ServiceDetail interface and populate 3 services with bilingual prose + FAQ</name>
  <files>src/lib/service-data.ts</files>
  <action>
1. Add optional `prose` field to `ServiceSection` interface:
```typescript
interface ServiceSection {
  id: string;
  title: LocaleString;
  prose?: LocaleString[];  // Array of paragraphs rendered before the feature list
  items: Array<{ text: LocaleString; icon?: string }>;
}
```

2. Add `faq` field to `ServiceDetail` interface:
```typescript
...
3. Add prose arrays and FAQ entries to the THREE services below. Keep ALL existing data (tagline, turnaroundDays, sections, items) untouched. Only ADD the new `prose` and `faq` fields.

---

**web-development** -- Add `prose` to the 'scope' section:
...
**web-apps** -- Add `prose` to the 'scope' section:
...
**seo** -- Add `prose` to the 'scope' section:
...
IMPORTANT: When writing service-data.ts, keep ALL existing data exactly as-is. Only ADD the `prose` arrays inside existing sections and the `faq` arrays to each service object. Do NOT modify existing tagline, turnaroundDays, section ids, titles, or items.
  </action>
  <verify>
    <automated>cd C:/Users/sylvi/Documents/projects/isio && npx astro check 2>&amp;1 | head -30</automated>
  </verify>
  <done>service-data.ts has ServiceSection.prose and ServiceDetail.faq interfaces. web-development, web-apps, and seo entries each have prose arrays in their sections and faq arrays with 3-4 Q&amp;A pairs each.</done>
</task>

<task type="auto">
  <name>Task 2: Update ServicePageLayout.astro to render prose paragraphs and FAQ section</name>
  <files>src/components/services/ServicePageLayout.astro</files>
  <action>
Update ServicePageLayout.astro to render the new prose and faq fields.

1. In the section loop (where each `detail.sections` entry is rendered), add prose rendering BEFORE the ServiceFeatureList:
...
2. After the turnaround block and before ServiceCTA, add the FAQ section:
...
The FAQ uses native HTML `<details>/<summary>` elements for accessible accordion behavior without JavaScript. The chevron rotates on open via `group-open:rotate-180`.

Keep all existing elements (hero section at top, turnaround block, ServiceCTA) exactly as they are. Only add prose rendering inside the section loop and FAQ block after turnaround.
  </action>
  <verify>
    <automated>cd C:/Users/sylvi/Documents/projects/isio && npx astro build 2>&amp;1 | tail -20</automated>
  </verify>
  <done>ServicePageLayout.astro renders prose paragraphs before feature lists in each section, and renders FAQ accordion after the turnaround block. Build succeeds with no errors.</done>
</task>
...
- `npx astro check` reports no type errors in service-data.ts or ServicePageLayout.astro
- Built HTML for `/servicii/website/` contains prose paragraphs and FAQ questions
- Built HTML for `/en/services/website/` contains English prose paragraphs and FAQ questions
</verification>

<success_criteria>
- ServiceDetail interface has optional `prose` (on ServiceSection) and `faq` fields
- web-development, web-apps, and seo services each have 3+ prose paragraphs per section and 3-4 FAQ entries
- ServicePageLayout renders prose as `<p>` elements and FAQ as `<details>` accordion
- Both RO and EN locales render the content correctly
- Build passes with zero errors
</success_criteria>

<output>
After completion, create `.planning/phases/11-content-expansion/11-01-SUMMARY.md`
</output>
